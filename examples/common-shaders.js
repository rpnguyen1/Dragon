...
update_GPU (context, gpu_addresses, uniforms, model_transform, material) {
    ...
    if (material.textured && material.textured.ready) {
      // Select texture unit 0 for the fragment shader Sampler2D uniform called "texture":
      context.uniform1i (gpu_addresses.textured, 0);
      // For this draw, use the texture image from correct the GPU buffer:
      material.textured.activate (context, 0);
    }

    if (material.texturea && material.texturea.ready) {
      // Select texture unit 0 for the fragment shader Sampler2D uniform called "texture":
...
const AnimatedShader = defs.AnimatedShader = // with texture maps
class AnimatedShader extends Shader {
  constructor (num_lights = 2) {
    super ();
    this.num_lights = num_lights;
}
...
fragment_glsl_code () {        // ********* FRAGMENT SHADER *********
  ...
  // Define sprite sheet size
  // const float SPRITE_COLS = cols + 0.0;  // 6 sprites per row
  // const float SPRITE_ROWS = rows + 0.0;  // 6 sprites per column
  float TOTAL_FRAMES = cols * rows;  // 36 frames
  ...
  void main() {
      ...
      // Determine sprite row and column (use float instead of int)
      float frame_x = mod(frame_index, cols); // Column index
      float frame_y = -floor(frame_index / rows); // Row index

      // Normalize frame positions within the sprite sheet (0-1 range)
      vec2 sprite_size = vec2(1.0 / cols, 1.0 / rows);
      vec2 sprite_offset = vec2(sprite_size.x * frame_x, sprite_size.y * frame_y);
...
send_material (gl, gpu, material) {
    ...
    gl.uniform1f (gpu.smoothness, material.smoothness);
    gl.uniform1f (gpu.speed, material.speed);
    gl.uniform1f (gpu.rows, material.rows);
    gl.uniform1f (gpu.cols, material.cols);
}
send_uniforms (gl, gpu, uniforms, model_transform) {
    const O = vec4 (0, 0, 0, 1), camera_center = uniforms.camera_transform.times (O).to3 ();
    gl.uniform3fv (gpu.camera_center, camera_center);
...
update_GPU (context, gpu_addresses, uniforms, model_transform, material) {
    const defaults    = {color: color (0, 0, 0, 1), ambient: 1, diffusivity: 1, specularity: 1, smoothness: 40, speed: 20.0, rows: 6.0, cols: 6.0};
    let full_material = Object.assign (defaults, material);
...
const PBRShader = defs.PBRShader = // with texture maps
...
fragment_glsl_code () {        // ********* FRAGMENT SHADER *********
  ...
  void main() {
      vec2 f_tex = f_tex_coord;

      // Sample textures
      vec3 albedo = texture2D(albedoMap, f_tex).rgb;   
      // float roughness = texture2D(roughnessMap, f_tex).r;
      float roughness = 1.0;
      // float metallic = texture2D(metallicMap, f_tex).r;
      float metallic = 1.0;
      // float ao = texture2D(aoMap, f_tex).r;
      float ao = texture2D(aoMap, f_tex).r;
      vec3 normal_from_map = texture2D(normalMap, f_tex).rgb * 2.0 - 1.0;

      // vec3 emission = texture2D(emissionMap, f_tex).rgb;
      // vec3 emission = texture2D(emissionMap, f_tex).rgb;
      
      // Normalize normal map
      vec3 N = normalize(normal_from_map);
      vec3 V = normalize(camera_center - vertex_worldspace); // View direction
      ...
      for (int i = 0; i < N_LIGHTS; i++) {
          vec3 L = normalize(light_positions_or_vectors[i].xyz - 
                            light_positions_or_vectors[i].w * vertex_worldspace);
          vec3 H = normalize(V + L);
          float distance = length(light_positions_or_vectors[i].xyz - vertex_worldspace);
          float attenuation = 1.0 / (1.0 + light_attenuation_factors[i] * distance * distance);
          ...
          vec3 kS = F;
          vec3 kD = vec3(1.0) - kS;
          kD *= 1.0 - metallic; // No diffuse reflection for metals
...
update_GPU (context, gpu_addresses, uniforms, model_transform, material) {
    ...
    if (material.metallicMap && material.metallicMap.ready) {
      context.uniform1i (gpu_addresses.metallicMap, 1);
      material.metallicMap.activate (context, 1);
    }
    if (material.roughnessMap && material.roughnessMap.ready) {
      context.uniform1i (gpu_addresses.roughnessMap, 2);
      material.roughnessMap.activate (context, 2);
    }
    if (material.normalMap && material.normalMap.ready) {
      context.uniform1i (gpu_addresses.normalMap, 3);
      material.normalMap.activate (context, 3);
    }
    if (material.aoMap && material.aoMap.ready) {
      context.uniform1i (gpu_addresses.aoMap, 4);
      material.aoMap.activate (context, 4);
    }
    if (material.emissionMap && material.emissionMap.ready) {
      context.uniform1i (gpu_addresses.emissionMap, 5);
      material.emissionMap.activate (context, 5);
    }
...
const PhongNShader = defs.PhongNShader = // with texture maps
class PhongNShader extends Shader {
  constructor (num_lights = 2) {
    super ();
    this.num_lights = num_lights;
}
...
vertex_glsl_code () {         // ********* VERTEX SHADER *********
  return this.shared_glsl_code () + `
varying vec2 f_tex_coord;
attribute vec3 position, normal, tangent;                            // Position is expressed in object coordinates.
attribute vec2 texture_coord;
...
void main() {
    gl_Position = projection_camera_model_transform * vec4( position, 1.0 );     // Move vertex to final space.
                                      // The final normal vector in screen space.
    N = normalize( mat3( model_transform ) * normal / squared_scale);
    // T = normalize( mat3(model_transform) * tangent / squared_scale);
    vec3 test =  vec3(1.0) + tangent;
    T = normalize(mat3(model_transform) * vec3(10.0) / squared_scale); 
    // Compute the bitangent using cross product
    B = cross(N, T);  
